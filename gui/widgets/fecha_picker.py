...
class FechaPicker(ctk.CTkFrame):
    """
    Widget de selección de fecha con dropdowns de Día, Mes y Año.
    Uso:
        picker = FechaPicker(parent)
        picker.pack(...)
        fecha_str = picker.get_fecha()  # retorna "YYYY-MM-DD"
    """
    def __init__(self, parent, fecha_inicial=None, **kwargs):
        ...
        # ── Día ─────────────────────────────────────────────
        ctk.CTkLabel(self, text="Día").grid(row=0, column=0, padx=(0,4))
        self.combo_dia = ctk.CTkComboBox(
            self, width=75, values=[str(d).zfill(2) for d in range(1, 32)],
            command=self._on_change
        )
        self.combo_dia.set(str(hoy.day).zfill(2))
        self.combo_dia.grid(row=1, column=0, padx=(0,8))

        ctk.CTkLabel(self, text="/").grid(row=1, column=1, padx=2)

        # ── Mes ─────────────────────────────────────────────
        ctk.CTkLabel(self, text="Mes").grid(row=0, column=2, padx=(0,4))
        self.combo_mes = ctk.CTkComboBox(
            self, width=130, values=MESES,
            command=self._on_change
        )
        self.combo_mes.set(MESES[hoy.month - 1])
        self.combo_mes.grid(row=1, column=2, padx=(0,8))

        ctk.CTkLabel(self, text="/").grid(row=1, column=3, padx=2)

        # ── Año ─────────────────────────────────────────────
        anio_actual = hoy.year
        anios = [str(a) for a in range(anio_actual - 5, anio_actual + 6)]
        ctk.CTkLabel(self, text="Año").grid(row=0, column=4, padx=(0,4))
        self.combo_anio = ctk.CTkComboBox(
            self, width=90, values=anios,
            command=self._on_change
        )
    ...
    def _on_change(self, _=None):
        """Corrige el día si excede los días del mes seleccionado."""
        try:
            ...
            # Si el día actual excede el máximo, ajustarlo
    ...
    def get_fecha(self):
        """Retorna la fecha seleccionada como string 'YYYY-MM-DD'."""
        try:
